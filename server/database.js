...
function Database()
{
	...
	this.find = function(tableName, id) {
		return tables[tableName].find((d) => (d.id === id))
	}

	this.push = function(tableName, doc) {
		let max_id = tables[tableName].reduce((prev, d) => Math.max(prev, d.id), 0)
	...
	this.update = function(tableName, id, update) {
		let doc = tables[tableName].find((d) => (d.id === id))

		return Object.assign(doc, update)
	}

	this.remove = function(tableName, id) {
		let index = tables[tableName].findIndex((d) => (d.id === id))
